...
import com.hzy.interfaceService.OrderService;
import com.hzy.model.UserOrder;

import com.hzy.serviceActivity.OrderServiceImpl;
import com.hzy.zservice.LoginActivity;
...
public class RetrofitUtil {
    ...
    public static String getOrderInfo(final Context context, final String a, final LinearLayout
            one) {
    ...
    public static Call<ResponseBody> getCall(OrderService service, int num, RequestBody body) {
        switch (num) {
            case 3:
                return service.getOrderdaiqiang(body);
            case 4:
    ...
    public static void getOrderDetail(final Context context, final ContextWrapper service,final int num, int[]
            orderNo, final String Stringa) {
        final OrderServiceImpl osi = new OrderServiceImpl();
        String Stringx = osi.getStr(orderNo);
        Logger.d("Stringx**************** 的值为："+Stringx );
        if ( null!=Stringx&&!Stringx.equals("")) {
            token = SharedPreferencesHelper.getInstance(context).getStringValue("token");
            Properties properties = AppProperties.getProperties(context);
            RequestBody body = RequestBody.create(okhttp3.MediaType.parse("application/json;" +
                    "charset=UTF-8"), Stringx);
            OrderService orderservice = provideClientApi(properties.getProperty("serverUrl")).create
                    (OrderService.class);
            Call<ResponseBody> call = getCall(orderservice, num, body);
            final String[] updateToken = {""};
            call.enqueue(new Callback<ResponseBody>() {
                @Override
                public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                    ...
                    try {
                        ...
                        if (null == jsonString || "".equals(jsonString)) {
                            ...
                            if (successStatus == true && msg.equals("ok")) {
                                ...
                                osi.onAccessSuccess(dingdanxinxi, service, jsonstringb,num);
                                //  System.out.println("jsonstring：" + dingdanxinxi);

                            } else if (successStatus == true && msg.equals("0")) {
                                Logger.d("**************** 无数据无数据无数据无数据为 无数据：" );
                            } else {
                                System.out.println("msg：1" + msg);
                                Intent intent = new Intent();
                                intent.setClass(context, LoginActivity.class);
